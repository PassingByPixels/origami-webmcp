...
export const videoBlock = {
    ...
    schemaComment: [
        ...
        'JSON shape: { provider: youtube|vimeo|loom|link|local, videoId, url: <the watch/share URL — for local, a deck-relative path>, title }',
        'provider, videoId and url MUST agree (the id the player loads = the video the url shows); link → videoId ""',
        'local → url is a path RELATIVE to the deck (e.g. media/intro.mp4): no scheme, no leading "/" or "//", no backslashes, no "..", and it must end .mp4/.m4v/.webm/.ogv/.mov; videoId ""; needs no capability (the runtime builds a native player)',
