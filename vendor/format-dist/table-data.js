...
/** A theme token or a fixed hex — the colour discipline every paintable field shares. */
const okColour = (x) => typeof x === 'string' && (FILL_TOKEN.test(x) || FILL_HEX.test(x));
/** The grid's extent as the address-bounded side-maps see it: the row count, and the widest of the
    declared columns and the actual rows. bake, merges, condFmt and filter all bound their addresses
    against exactly this, so it is derived in one place rather than four. */
...
const checkFormula = (where, key, val, bad) => {
...
const checkFormat = (where, key, val, bad) => {
    if (!isMap(val)) {
...
const checkStyle = (where, key, val, bad) => {
    if (!isMap(val)) {
...
const a1Map = (field, val, bad, each) => {
    if (!isMap(val)) {
...
/** Optional stable per-SHEET id (lazily minted on first chart link to this sheet — see the
    ...
    `hidden` is the sheet-level Present-visibility flag. It mirrors the `wrap` discipline — only the
    literal `true` is ever stored (a shown sheet omits it → byte-identical), so anything else,
    including `false`, is rejected. Sheet-scoped, so a tab entry's data may carry it too. */
function checkSheetIdentity(d, bad) {
...
function checkFormulaMap(d, bad) {
    ...
    for (const [k, val] of Object.entries(d.formulas)) {
        ...
        checkFormula('formulas.value', k, val, bad);
    }
}
function checkNamedMap(d, bad) {
    ...
    for (const [k, val] of Object.entries(d.named)) {
        ...
        checkFormula('named.value', k, val, bad);
    }
}
...
function checkSourceFieldNames(so, bad) {
...
function checkSourceParams(so, bad) {
...
function checkSource(d, bad) {
    ...
    checkSourceParams(so, bad);
}
function checkOrefreshed(d, bad) {
...
function checkColumnEntry(o, i, bad) {
    if (o.format !== undefined)
        checkFormat('column.format', `column ${i}`, o.format, bad);
...
function checkColumnDisplay(d, bad) {
    if (!Array.isArray(d.columns))
        return;
    d.columns.forEach((c, i) => checkColumnEntry((c ?? {}), i, bad));
}
function checkCellMaps(d, bad) {
    if (d.cellFormats !== undefined)
        a1Map('cellFormats', d.cellFormats, bad, (k, v0) => checkFormat('cellFormats.value', k, v0, bad));
    if (d.cellStyles !== undefined)
        a1Map('cellStyles', d.cellStyles, bad, (k, v0) => checkStyle('cellStyles.value', k, v0, bad));
    if (d.cellNames === undefined)
        return;
    a1Map('cellNames', d.cellNames, bad, (k, v0) => {
...
function checkRowHeights(d, bad) {
...
/** `rules` — the per-axis formula-fill templates. (Distinct from `condFmt`, the paint rules.) */
function checkRuleTemplates(d, bad) {
    ...
    for (const axis of ['cols', 'rows']) {
        ...
        for (const [k, v0] of Object.entries(set)) {
            ...
            checkFormula(`rules.${axis}.value`, k, v0, bad);
        }
    }
}
function checkRuleOverrides(d, bad) {
...
function checkTotals(d, bad) {
    ...
    if (totals.on !== undefined && typeof totals.on !== 'boolean')
        bad('totals.on', 'totals.on must be a boolean');
    if (totals.fns === undefined)
        return;
    if (!isMap(totals.fns)) {
        bad('totals.fns', 'totals.fns must be an object map of colIndex -> function');
        return;
    }
    for (const [k, v0] of Object.entries(totals.fns)) {
        if (!INDEX_KEY.test(k))
...
function checkCropRect(r, rule, nr, nc, bad) {
    if (!isMap(r)) {
...
function checkBakeView(view, i, names, nr, nc, bad) {
    if (!isMap(view)) {
    ...
    checkCropRect(view.rect, `bake.views[${i}].rect`, nr, nc, bad);
}
/** additive named views: a non-empty array of { name, rect }; names non-empty ≤NAME_MAX + UNIQUE.
    Absent `views` ⇒ none of this runs (back-compat). Returns the names, for `active` to match. */
function checkBakeViews(bake, nr, nc, bad) {
    const names = new Set();
    if (bake.views === undefined)
        return names;
    if (!Array.isArray(bake.views) || bake.views.length === 0) {
        bad('bake.views', 'bake.views must be a non-empty array of { name, rect }');
        return names;
    }
    bake.views.forEach((view, i) => checkBakeView(view, i, names, nr, nc, bad));
    return names;
}
function checkBake(d, bad) {
    if (d.bake === undefined)
        return;
    if (!isMap(d.bake)) {
    ...
    const { nr, nc } = gridExtent(d);
    if (bake.rect !== undefined)
        checkCropRect(bake.rect, 'bake.rect', nr, nc, bad);
    const names = checkBakeViews(bake, nr, nc, bad);
    if (bake.active === undefined)
        return;
    ...
    else if (typeof bake.active !== 'string' || !names.has(bake.active))
        bad('bake.active', 'bake.active must name an existing view');
}
/** One merge region: a well-formed A1 range of ≥2 cells, in-bounds, and not overlapping one
    already seen. Records its rect in `seen` so the next entry is tested against it. */
function checkMergeEntry(m, i, seen, nr, nc, bad) {
    if (typeof m !== 'string') {
...
function checkMerges(d, bad) {
    ...
    const { nr, nc } = gridExtent(d);
    const seen = [];
    d.merges.forEach((m, i) => checkMergeEntry(m, i, seen, nr, nc, bad));
}
const requireHexRamp = (rule, i, _kind, bad) => {
...
const requireNumericValue = (rule, i, kind, bad) => {
    if (typeof rule.value !== 'number' || !Number.isFinite(rule.value))
        bad('condFmt.value', `condFmt[${i}] ${kind} needs a numeric "value"`);
};
const requireNonEmptyText = (rule, i, _kind, bad) => {
    if (typeof rule.text !== 'string' || rule.text.trim() === '')
        bad('condFmt.text', `condFmt[${i}] eq needs a non-empty "text"`);
};
const requirePositiveCount = (rule, i, kind, bad) => {
    if (typeof rule.n !== 'number' || !Number.isInteger(rule.n) || rule.n < 1)
        bad('condFmt.n', `condFmt[${i}] ${kind} needs an integer "n" ≥ 1`);
};
/* gt/lt and top/bot are one contract each, written once and keyed twice: the two members of each
   pair differ only in the comparison the RUNTIME makes, never in what the rule may carry. */
const COMPARE_SPEC = {
    require: requireNumericValue,
    forbids: ['text', 'n', 'from', 'to'],
    forbidRule: 'condFmt.field',
    forbidDetail: (i, kind) => `condFmt[${i}] ${kind} takes only value + fill/color`,
    needsPaint: true,
};
const RANK_SPEC = {
    require: requirePositiveCount,
    forbids: ['value', 'text', 'from', 'to'],
    forbidRule: 'condFmt.field',
    forbidDetail: (i, kind) => `condFmt[${i}] ${kind} takes only n + fill/color`,
    needsPaint: true,
};
/* A Map, not a plain object: `kind` comes straight off the parsed file, so an object literal would
   answer a lookup for "constructor" or "toString" with something off Object.prototype. */
const COND_KIND_SPECS = new Map([
    ['scale', {
            require: requireHexRamp,
            forbids: ['fill', 'color', 'value', 'text', 'n'],
            forbidRule: 'condFmt.scale',
            forbidDetail: (i) => `condFmt[${i}] scale takes only from/to (no fill/color/value/text/n)`,
            needsPaint: false,
        }],
    ['gt', COMPARE_SPEC],
    ['lt', COMPARE_SPEC],
    ['eq', {
            require: requireNonEmptyText,
            forbids: ['value', 'n', 'from', 'to'],
            forbidRule: 'condFmt.field',
            forbidDetail: (i) => `condFmt[${i}] eq takes only text + fill/color`,
            needsPaint: true,
        }],
    ['top', RANK_SPEC],
    ['bot', RANK_SPEC],
    ['dupes', {
            forbids: ['value', 'text', 'n', 'from', 'to'],
            forbidRule: 'condFmt.field',
            forbidDetail: (i) => `condFmt[${i}] dupes takes only fill/color`,
            needsPaint: true,
        }],
]);
// per-kind: require what the kind needs, forbid what it can't carry. An unrecognised kind has
// already been reported by the kind check, so there is nothing further to say about its fields.
function checkCondFmtKind(rule, i, kind, bad) {
    const spec = typeof kind === 'string' ? COND_KIND_SPECS.get(kind) : undefined;
    if (!spec)
        return;
    spec.require?.(rule, i, kind, bad);
    if (spec.forbids.some((f) => rule[f] !== undefined))
        bad(spec.forbidRule, spec.forbidDetail(i, kind));
    if (spec.needsPaint && rule.fill === undefined && rule.color === undefined) {
        bad('condFmt.paint', `condFmt[${i}] ${kind} needs a fill or a color`);
    }
}
// range: well-formed A1 range, in-bounds of the grid
function checkCondFmtRange(rule, i, nr, nc, bad) {
...
function checkCondFmtRule(rule, i, nr, nc, bad) {
    if (!isMap(rule)) {
    ...
    checkCondFmtRange(rule, i, nr, nc, bad);
    ...
    checkCondFmtKind(rule, i, kind, bad);
}
...
function checkCondFmt(d, bad) {
    ...
    const { nr, nc } = gridExtent(d);
    d.condFmt.forEach((rule, i) => checkCondFmtRule(rule, i, nr, nc, bad));
}
/** filter.cols: a non-empty, deduped, in-bounds list of column indices. */
function checkFilterCols(fr, nc, bad) {
...
function validateSheet(d, prefix, v) {
    ...
    checkSheetIdentity(d, bad);
    checkFormulaMap(d, bad);
    checkNamedMap(d, bad);
    checkSource(d, bad);
    checkOrefreshed(d, bad);
    checkColumnDisplay(d, bad);
    checkCellMaps(d, bad);
    checkRowHeights(d, bad);
    checkRuleTemplates(d, bad);
    checkRuleOverrides(d, bad);
    checkKpis(d, bad);
    checkTotals(d, bad);
    checkBake(d, bad);
    checkMerges(d, bad);
...
/** Optional stable block id — lazily minted on first chart link (present → shape check, absent →
    skip) — and the block-level DISPLAY name (see TableData.name): presentation text, NOT the old
    cross-ledger link identifier, so any printable string is fine (spaces/unicode ok), only bounded
    like `id`/`sid`. Non-empty ≤ NAME_MAX; a shown ledger omits it when unnamed → byte-identical.
    Reject '' / over-length / non-string (the empty case is a CLEAR at the editor, never stored). */
function checkBlockIdentity(d, bad) {
...
/** One tab entry: { name, data }, the name unique across the strip, the data a FULL sheet
    (recursed) minus the block-level fields. */
function checkTabEntry(tab, i, names, v, bad) {
    if (!isMap(tab)) {
    ...
    validateSheet(sheet, `table.tabs[${i}]`, v);
}
...
function checkTabStrip(d, v, bad) {
    ...
    if (typeof d.tabName === 'string')
        names.add(d.tabName);
    tabsArr.forEach((tab, i) => checkTabEntry(tab, i, names, v, bad));
}
...
export function validateTableData(data) {
    ...
    validateSheet(d, 'table', v);
    checkBlockIdentity(d, bad);
    checkTabStrip(d, v, bad);
    checkTabPos(d, bad);
    return v;
}
