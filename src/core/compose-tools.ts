  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          blocks: {
            type: 'array',
            description: `The card's content, in order. Each entry names exactly ONE of: ${KIND_LIST} — e.g. { "chart": { "type": "bar", "labels": ["Q1","Q2"], "series": [{ "name": "Revenue", "color": "#4A8CC4", "values": [12,19] }], "yMax": null }, "caption": "Revenue by quarter" }, or { "stats": [{ "value": "48", "label": "Decks shipped" }] }, or { "text": "<p class=\\"lede\\">A line of copy.</p>" }. SIZE: a venn | flow | graph | gantt | table entry may add "width" and/or "height", whole CSS px (width ${SIZE_RANGE.width[0]}-${SIZE_RANGE.width[1]}, height ${SIZE_RANGE.height[0]}-${SIZE_RANGE.height[1]}) — leave them out and the card picks a height that fits 1280x720 (a default graph overflows it). A size on a chart or a draw block is REFUSED: the runtime ignores it, and each of those two is sized inside its own JSON instead (plotHeight, wpct)`,
            items: { type: 'object' },
          },
        },
