...
test('registers every tool on document.modelContext and reports it', async ({ page }) => {
  ...
  await expect(page.getByTestId('mcp-status')).toHaveText('WebMCP: connected via document.modelContext — 31 tools');
  ...
  expect(defs.filter((d: any) => d.annotations?.readOnlyHint)).toHaveLength(11);
...
test('falls back to navigator.modelContext when document has none', async ({ page }) => {
  ...
  await page.goto('/folio/index.html');
  await expect(page.getByTestId('mcp-status')).toHaveText('WebMCP: connected via navigator.modelContext — 31 tools');
  expect(await page.evaluate(() => (window as any).__mcp_navigator.registered.length)).toBe(31);
});
...
test('prefers document.modelContext when BOTH surfaces exist', async ({ page }) => {
  ...
  expect(await page.evaluate(() => (window as any).__mcp_document.registered.length)).toBe(31);
