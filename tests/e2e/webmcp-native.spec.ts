...
test.describe('native WebMCP in the installed stable Chrome', () => {
  ...
  test('the app registers all 37 tools on Chrome\'s own modelContext', async () => {
    ...
    try {
      ...
      await expect(c.page.getByTestId('mcp-status')).toHaveText('WebMCP: connected via document.modelContext — 37 tools');
