...
test.describe('native WebMCP in the installed stable Chrome', () => {
  ...
  test('the app registers all 21 tools on Chrome\'s own modelContext', async () => {
    ...
    try {
      ...
      await expect(c.page.getByTestId('mcp-status')).toHaveText('WebMCP: connected via document.modelContext — 21 tools');
      ...
      expect(tools.map((t: any) => t.name).sort()).toEqual([
        ...
        'set_fold_type', 'set_header', 'write_chunk',
      ]);
